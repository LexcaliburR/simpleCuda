...
cudaError_t ArrayAddV1Launch(float* array1, float* array2, size_t size,
                             float* result, cudaStream_t stream = nullptr)
{
...
int main()
{
    ...
    CUDA_CHECK(cudaMemcpy(
        array1_d, array1_h, size * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(
        array2_d, array2_h, size * sizeof(float), cudaMemcpyHostToDevice));
